...
export default function DriverRegister() {
  ...
  const validate = () => {
    ...
    if (!plate) e.plateNumber = 'Your van\'s plate number is required.';
    else if (!PLATE_RE.test(plate)) e.plateNumber = 'Use letters, numbers, spaces, or dashes only (4–15 chars).';

    if (!form.capacity) e.capacity = 'Van capacity is required.';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      payload.append('plateNumber', form.plateNumber.trim().toUpperCase());
      ...
      setAssignedPlate(data?.driver?.assignedVan?.plateNumber ?? form.plateNumber.trim().toUpperCase());
      setSubmitted(true);
    } catch (err) {
      ...
      <div className="min-h-screen bg-slate-100 flex items-center justify-center px-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <p className="text-sm text-slate-500 mt-3 leading-relaxed">
            An admin will review your details, license photo, and van <strong>{assignedPlate}</strong>.
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 sm:p-8">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-500 mt-2">
            Register yourself and your van — an admin reviews every application before it's active.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b pb-1">Your Details</h3>
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b pb-1">Your Van</h3>
            ...
            <FormField label="Plate Number" required error={errors.plateNumber}>
              <input
                ...
                placeholder="e.g. ABC-1234" autoComplete="off" maxLength={15}
                className={`${inputCls(errors.plateNumber)} uppercase`}
              />
            </FormField>

            <FormField label="Capacity" required error={errors.capacity} hint="Seats (1–30)">
              <input
